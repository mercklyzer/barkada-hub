...
export function useImpostorGame(): UseImpostorGame {
  ...
  const startGame = useCallback(
    async (settings: GameSettings) => {
      ...
      setError(null)
      try {
        const words = await fetchImpostorWords(settings.category, settings.language, 20)
  ...
  const onRevealDone = useCallback(() => {
    setSession((prev) => {
      ...
      setTimeout(() => {
        if (allRevealed) {
          setSession((s) => (s ? { ...s, currentPlayerIndex: 0, currentRound: 1 } : s))
  ...
  const submitVote = useCallback((vote: Vote) => {
    setSession((prev) => {
      ...
      if (nextPlayerIndex < prev.players.length) {
        return { ...prev, votes: newVotes, currentPlayerIndex: nextPlayerIndex }
      }

      // All players voted — tally
      const tallies: Record<string, number> = {}
      for (const v of newVotes) {
        tallies[v.suspectId] = (tallies[v.suspectId] ?? 0) + 1
      }
  ...
  const submitImpostorGuess = useCallback((guess: string) => {
    setSession((prev) => {
      ...
      const outcome: ImpostorSession['outcome'] = correct ? 'impostor_wins' : 'team_wins'
